...
public class Tests_2104
{
    // MUST exist within project, otherwise a Compile Time Error will be thrown.
    private static readonly Assembly ProjectAssembly = typeof(StartUp).Assembly;

    [Test]
    public void Validate_RegisterUser_SuccessfullyRegistered_Admin()
    {
        var controller = CreateObjectInstance(GetType("Controller"));

        var methodArgs = new object[] { "TimBurton", "tim_bb@email.com", true };

        var errorResult = InvokeMethod(controller, "RegisterUser", methodArgs);

        Assert.That(errorResult, Is.EqualTo("Admin TimBurton is successfully registered with data access."));
    }

...
public class Tests_201
{
    // MUST exist within project, otherwise a Compile Time Error will be thrown.
    private static readonly Assembly ProjectAssembly = typeof(StartUp).Assembly;

    [Test]
    public void ValidateCreateTeam_InvalidParameters()
    {
        var controller = CreateObjectInstance(GetType("Controller"));

        var methodArgs = new object[] { string.Empty};
        var errorResult = InvokeMethod(controller, "JoinChampionship", methodArgs);

        Assert.That(errorResult, Is.EqualTo("Team name cannot be null or empty."));
    }

...
[TestFixture]
public class Tests_109
{
    // MUST exist within project, otherwise a Compile Time Error will be thrown.
    private static readonly Assembly ProjectAssembly = typeof(StartUp).Assembly;

    [Test]
    public void ValidateTeamMethods()
    {
        var type = GetType("Team");

        var methods = new[]
        {
            new Method(typeof(void), "SignWith", typeof(IManager)),
            new Method(typeof(void), "GainPoints", typeof(int)),
            new Method(typeof(void), "ResetPoints"),
        };

...
[TestFixture]
public class Tests_110
{
    // MUST exist within project, otherwise a Compile Time Error will be thrown.
    private static readonly Assembly ProjectAssembly = typeof(StartUp).Assembly;

    [Test]
    public void ValidateTeamRepositoryMethods()
    {
        var type = GetType("TeamRepository");

        var methods = new[]
        {
            new Method(typeof(void), "Add", typeof(ITeam)),
            new Method(typeof(bool), "Remove", typeof(string)),
            new Method(typeof(bool), "Exists", typeof(string)),
            new Method(typeof(ITeam), "Get", typeof(string))
        };

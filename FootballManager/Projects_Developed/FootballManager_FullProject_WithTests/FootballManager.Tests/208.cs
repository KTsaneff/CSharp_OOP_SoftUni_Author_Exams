...
public class Tests_208
{
    // MUST exist within project, otherwise a Compile Time Error will be thrown.
    private static readonly Assembly ProjectAssembly = typeof(StartUp).Assembly;

    [Test]
    public void Validate_SignManager_ManagerAlreadySignedContractWithAnotherTeam()
    {
        ...
        var methodArgs2 = new object[] { "Manchester United" };
        InvokeMethod(controller, "JoinChampionship", methodArgs2);

        var methodArgs3 = new object[] { "Liverpool", "AmateurManager", "NewManager Signed" };
        InvokeMethod(controller, "SignManager", methodArgs3);

        var methodArgs4 = new object[] { "Manchester United", "AmateurManager", "NewManager Signed" };

        var actualResult = InvokeMethod(controller, "SignManager", methodArgs4);
        var expectedResult = "Manager NewManager Signed is already assigned to another team.";
